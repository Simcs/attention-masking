...
def measure_performances(id_maha, ood_maha, seed=0):
    ...
    pr, re, aupr_score = aupr(test_y, ood_scores)
    fpr95 = fpr_at_95_tpr(test_y, ood_scores)
    print(seed, ':', auroc_score, aupr_score, fpr95)
...
def compute_classwise_mahalanobis_ood_scores(args):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'

    print('loading train statistics...')
    stat_fn = get_statistics_filename('k400', 'train', args)
    stat_fn = stat_fn.parent / f'{stat_fn.stem}_statistics_classwise.pkl'
    stat_fn = './data/kinetics/k400_train_statistics_classwise_0.4.pkl'
    # stat_fn = './data/kinetics/statistics/masekd/k400_train_statistics_classwise_empirical.pkl'
    # stat_fn = 'data/kinetics/statistics/masked/k400_train/spatiotemporal_max_0.5_lt_threshold_0.01_lt_threshold_0.05_statistics_classwise.pkl'
    
    # stat_fn = '/home/simc/workspace/OOD-with-ViT/data/kinetics/k400_train_statistics_classwise.pkl'
    ...
    # k400_embeddings_filename = '/home/simc/workspace/OOD-with-ViT/data/kinetics/embeddings/masked/k400_val/spatiotemporal_max_0.5_lt_threshold_0.01_random_None_center.jsonl'
    k400_embeddings_filename = '/home/simc/workspace/OOD-with-ViT/data/kinetics/embeddings/masked/k400_val/spatiotemporal_max_0.5_lt_threshold_0.01_lt_threshold_0.05.jsonl'
    
    # k400_embeddings_filename = './data/kinetics/embeddings/masked/k400_val/spatiotemporal_max_0.5_lt_threshold_0.01_random_None_skip.jsonl'
    k400_embeddings_filename = './data/kinetics/embeddings/masked/k400_val/spatial_max_0.5_lt_threshold_0.01.jsonl'
    # k400_embeddings_filename = './data/kinetics/embeddings/masked/k400_val/spatiotemporal_max_0.5_lt_threshold_0.01_random_None_random.jsonl'
    ...
    print('computing k400 classwise mahalanobis distances...')
    ...
    for k400_features in k400_val.chunk(2, dim=0):
        ...
        for mean, precision in tqdm(zip(means, precisions), total=400):
        ...
        k400_features = k400_features.to('cpu')
        k400_gaussian_scores = torch.cat(k400_gaussian_scores, dim=1)
        k400_cw_maha, _ = k400_gaussian_scores.min(dim=1)
        k400_mahalanobis_distances.extend(k400_cw_maha.numpy())
    ...
    # k600_embeddings_filename = '/home/simc/workspace/OOD-with-ViT/data/kinetics/embeddings/masked/k600_val/spatiotemporal_max_0.5_lt_threshold_0.01_random_None_center_deduplicated.jsonl'
    # k600_embeddings_filename = './data/kinetics/embeddings/masked/k600_val/spatiotemporal_max_0.5_lt_threshold_0.01_random_None_skip_deduplicated.jsonl'
    k600_embeddings_filename = './data/kinetics/embeddings/masked/k600_val/spatial_max_0.5_lt_threshold_0.01_deduplicated.jsonl'
    # k600_embeddings_filename = './data/kinetics/embeddings/masked/k600_val/spatiotemporal_max_0.5_lt_threshold_0.01_random_None_random_deduplicated.jsonl'
    ...
    id_label, ood_label, detector = 'K400', 'K600_excl', 'Mahalanobis'
    save_histogram(
        id=k400_mahalanobis_distances, 
        ood=k600_mahalanobis_distances, 
        id_label=id_label, 
        ood_label=ood_label, 
        detector=detector, 
    ...
    # max_ood_score = max(ood_scores)
    # ood_scores = [i / max_ood_score for i in ood_scores]
    print('# of k400 and k600 embeddings:', len(ood_scores))
    ...
    result_filename = result_filename.parent / (result_filename.stem + '_cw_maha' + result_filename.suffix)
    ...
    auroc_filename = image_filename.parent / (image_filename.name + '_cw_maha_auroc.png')
    save_roc_curve(fpr, tpr, auroc_filename)
    aupr_filename = image_filename.parent / (image_filename.name + '_cw_maha_aupr.png')
    ...
    k700_embeddings_filename = k700_emb_fn.parent / (k700_emb_fn.stem + '_deduplicated' + k700_emb_fn.suffix)
    # k700_embeddings_filename = './data/kinetics/embeddings/masked/k700-2020_val/spatiotemporal_max_0.5_lt_threshold_0.01_random_None_skip_deduplicated.jsonl'
    k700_embeddings_filename = './data/kinetics/embeddings/masked/k700-2020_val/spatial_max_0.5_lt_threshold_0.01_deduplicated.jsonl'
    # k700_embeddings_filename = './data/kinetics/embeddings/masked/k700-2020_val/spatiotemporal_max_0.5_lt_threshold_0.01_random_None_random_deduplicated.jsonl'
    # k700_embeddings_filename = '/home/simc/workspace/OOD-with-ViT/data/kinetics/embeddings/masked/k700-2020_val/spatiotemporal_max_0.5_lt_threshold_0.01_random_None_center_deduplicated.jsonl'
    k700_original = []
    ...
    k700_mahalanobis_distances = []
    for k700_features in k700_original.chunk(2, dim=0):
        k700_gaussian_scores = []
        k700_features = k700_features.to(device)
        for mean, precision in tqdm(zip(means, precisions), total=400):
            zero_f = k700_features - mean
            gau_term = torch.mm(torch.mm(zero_f, precision), zero_f.t()).diag()
            k700_gaussian_scores.append(gau_term.cpu().view(-1, 1))
        k700_original = k700_original.to('cpu')
        k700_gaussian_scores = torch.cat(k700_gaussian_scores, dim=1)
        k700_cw_maha, _ = k700_gaussian_scores.min(dim=1)
        k700_mahalanobis_distances.extend(k700_cw_maha.numpy())
    # k700_mahalanobis_distances = k700_mahalanobis_distances / k700_mahalanobis_distances.max(dim=0)
    ...
    # max_ood_score = max(ood_scores)
    # ood_scores = [i / max_ood_score for i in ood_scores]
    print('# of k400 and k700 embeddings:', len(ood_scores))

    id_label, ood_label, detector = 'K400', 'K700_excl', 'Mahalanobis'
    save_histogram(
        id=k400_mahalanobis_distances, 
        ood=k700_mahalanobis_distances, 
        id_label=id_label, 
        ood_label=ood_label, 
        detector=detector, 
        path=f'histograms/{id_label}+{ood_label}_{detector}_masked.png',
...
if __name__ == "__main__":
    ...
    compute_classwise_mahalanobis_ood_scores(args)