...
ucf_fold, hmdb_fold = args.ucf_fold, args.hmdb_fold
...
hmdb_embedding_fn = f'./data/hmdb51/embeddings/original/hmdb51_{hmdb_fold}_ucf101_{ucf_fold}_all_embeddings.jsonl'
print('hmdb embedding:', hmdb_embedding_fn)
hmdb51_embs = []
with open(hmdb_embedding_fn, 'r') as f:
    for line in tqdm(f):
        emb_js = json.loads(line)
...
print('computing gaussian scores finished.')
print('# of k600 embeddings:', len(hmdb51_mahalanobis_distances))
...
with open('./result/ood_scores/video/original/ucf101_vs_hmdb51_maha.jsonl', 'w') as f:
    f.write(json.dumps({
        'auroc': ucf_hmdb_auroc_score,
        'aupr': ucf_hmdb_aupr_score,
        'fpr95': ucf_hmdb_fpr95,
    }))
save_roc_curve(fpr, tpr, './result/images/ucf101_vs_hmdb51/maha_auroc.png')
save_precision_recall_curve(pr, re, './result/images/ucf101_vs_hmdb51/maha_aupr.png')