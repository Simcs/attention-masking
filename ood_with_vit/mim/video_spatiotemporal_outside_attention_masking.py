...
class VideoSpatiotemporalOutsideAttentionMaskingHooker(AttentionMaskingHooker):
    ...
    def compute_attention_masks(self, spatial_attention_maps, temporal_attention_maps):
        ...
        mask_idx = torch.Tensor([0, 1, 6, 7]).to(torch.int64)
        mask_idx = mask_idx.repeat(temporal_attention_maps.size(0), 1)
        temporal_masks = _masks_from_mask_indices(temporal_attention_maps, mask_idx)
